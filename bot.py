...
var botConfigs = {
    token: "NTI2MjIwNTA0OTY1NTc4NzYy.XMsE-Q.FAbbJKZNxgML3HHMckHLDdoG_io",
    prefix: "m!",
    gameStatus: "ฮั่นแน่ชอบเร่งเครื่องเปล่า?",
    statusType: "PLAYING",
    commands: [],
    plugins: [
    {
...
client.on("message", async function(message) {
    ...
    if (command === "pause" && botConfigs.plugins[8].activated == true) {
        ...
        message.channel.send(`⏸ โมบายล์ได้ทำการหยุดเพลง ${fetched.queue[0].songTitle}`);
    }
    ...
    if (command === "play" && botConfigs.plugins[8].activated == true) {
        ...
        if (!data.dispatcher) play(client, ops, data);
            ...
            message.channel.send(`🎼 โมบายล์ได้เพื่มเพลง: ${info.title} | 👥 ชื่อคนขอเพลง: ${message.author.tag}`);
        }
    ...
    if (command === "queue" && botConfigs.plugins[8].activated == true) {
        ...
        let resp = `__**กำลังเล่นเพลง 🎧**__\n**${nowPlaying.songTitle}** -- ** 👥 ชื่อคนขอเพลง:** *${nowPlaying.requester}*\n\n__**🎼 คิวเพลง**__\n`;

        for (var i = 1; i < queue.length; i++) {
             resp += `${i}. **${queue[i].songTitle}** -- ** 👥 ชื่อคนขอเพลง:** *${queue[i].requester}*\n`;
        }
    ...
    if (command === "resume" && botConfigs.plugins[8].activated == true) {
        ...
        message.channel.send(`▶ โมบายล์ได้ทำการเปิดเพลงต่อ ${fetched.queue[0].songTitle}`);
    }
    ...
    if (command === "skip" && botConfigs.plugins[8].activated == true) {
        ...
        if (fetched.queue[0].voteSkips.length >= required) {
            message.channel.send('✅ โมบายล์ทำการข้ามเพลงนี้เรียบร้อยแล้ว!');
...
async function play(client, ops, data) {
    ...
    client.channels.get(data.queue[0].announceChannel).send(`🎧 โมบายล์กำลังเล่นเพลง: ${data.queue[0].songTitle} | 👥 ชื่อคนขอเพลง: ${data.queue[0].requester}`);
